          ...
          child: Column(
            ...
            children: [
              ...
              ApplyButton(
                  title: Strings.apply,
                  onPressed: () {
                    if (_formKey.currentState!.validate() && global.store.get('selectedDay') != null) {

                      BlocProvider.of<GoalListBloc>(context)
                        ..add(AddGoalEvent(
                            goal: GoalModel(
                          title: global.store.get('Goal Title'),
                          calendar: global.store.get('selectedDay').toString(),
                          category: global.store.get('Goal category') ?? 'test',
                          isRemind:
                              ((global.store.get('Goal isReminded') == false)? 0: 1),
                          isCompleted: 1,
                                    term: helper.daysBetween(global.store.get('selectedDay').toString()),
                        )));
